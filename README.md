...
- **Google Reviews** (`pages/integrations.js`, `pages/leads.js`, `pages/dashboard.js`, `lib/googlePlaces.js`, `lib/resendEmail.js`, `lib/twilioSms.js`, `pages/api/review-request.js`, `pages/api/google-reviews-refresh.js`) — review request campaigns (email/SMS with a direct link to leave a Google review, sent automatically when a lead is marked "sold" or on demand from `/leads`) plus a live Google Rating card on the dashboard. Uses a plain Place ID + Maps Platform API key rather than the gated Business Profile APIs — no manual Google approval needed. Every request sends the identical neutral message to every lead: Google's Rating Manipulation policy (reinforced April 2026) bans "review gating" — filtering who gets asked, or what they're asked to say, based on predicted sentiment — so there's deliberately no code path for that here. SMS sending is gated per-lead on a `sms_consent` checkbox (TCPA); email isn't. Uses Resend for email and Twilio for SMS — both called via raw `fetch`, no new SDKs added.
- Database schema for clients, tiers, opportunities, unified content items, leads, and vertical playbooks (`supabase/schema.sql` + `supabase/migrations/002_phase1.sql` + `supabase/migrations/003_google_integrations.sql` + `supabase/migrations/005_lead_webhooks.sql` + `supabase/migrations/006_google_reviews.sql`), with row-level security so each client only sees their own data
...
- A public lead-capture form/embed for `pages/api/leads.js` — it's live but nothing calls it yet (Phase 3, needs subdomain routing first). Until then, `leads.sms_consent` is set manually via the checkbox on `/leads`.
...
6. Then run `supabase/migrations/006_google_reviews.sql` (adds `clients.google_place_id`/rating fields, `leads.sms_consent`/`review_requested_at`, and the `review_requests` log table)
7. Optionally run `supabase/seed_demo.sql` (edit the `:client_id` placeholder first) to seed the NationalCarDeals demo opportunities

### 3. Set up environment variables
```bash
cp .env.local.example .env.local
```
Fill in the Supabase and Anthropic values in `.env.local`. For Google Analytics/Search Console, see the setup steps commented in `.env.local.example` — one-time Google Cloud service account creation, then per-client access grants in GA4/Search Console. For Stripe, see the setup steps commented above the `STRIPE_*` block in the same file — one-time Stripe Dashboard setup (API key, 3 prices, webhook endpoint, Customer Portal config), test mode is fine to start with. For Google Reviews, see the setup steps commented above the review-request block — a Maps Platform API key (self-serve, no manual approval, separate from the GA4/Search Console service account) plus a Resend account (email) and/or Twilio account (SMS).
...
Suggested smoke test: sign up a fresh account → Business Brain onboarding saves and redirects to the dashboard → "Generate opportunities" produces rows on `/opportunities` → "Create content" on one produces a draft with a visible SEO score on `/content/[id]` → Approve → Schedule shows it on `/calendar` → posting to `/api/leads` (or inserting a row manually) shows up on `/leads` and `/analytics` → on `/integrations`, paste a GA4 Property ID and Search Console Site URL and confirm "Save & Test" reports success, then confirm real numbers show up on `/dashboard` and `/analytics` → still on `/integrations`, paste a Zapier "Catch Hook" (or webhook.site) URL into Lead Webhook, click "Save & Test", confirm it shows "Test event delivered ✓" and the test payload actually arrived at the URL, then submit another lead via `/api/leads` and confirm that one arrives too → on `/settings`, subscribe to a plan with Stripe's test card `4242 4242 4242 4242` (any future expiry/CVC), confirm you land back on `/settings?checkout=success` and the plan flips to that tier/`active` within a few seconds, then click "Manage Billing" and confirm Stripe's portal opens → still on `/integrations`, paste your Google Place ID into Google Reviews and confirm "Save & Test" shows your real rating/review count, then on `/leads` check the SMS OK box on a lead with a phone number (your own, for testing) and click "Request review" — confirm an email (and text, if configured) arrives with a working Google review link, then mark a different lead "sold" and confirm a review request fires automatically.

### Testing Google Reviews locally
1. **Place ID + rating**: In Google Cloud Console, enable "Places API (New)", create an API key restricted to it, and enable billing on the project (Places API isn't free, but rating/review-count lookups are inexpensive — see [Google's pricing](https://developers.google.com/maps/documentation/places/web-service/usage-and-billing)). Add it as `GOOGLE_MAPS_API_KEY`. Find any business's Place ID with Google's [Place ID Finder](https://developers.google.com/maps/documentation/places/web-service/place-id).
2. **Email**: Create a [Resend](https://resend.com) account, verify a sending domain, create an API key. Add `RESEND_API_KEY` and `REVIEW_REQUEST_FROM_EMAIL` (e.g. `Envero <reviews@yourdomain.com>`).
3. **SMS** (optional): Create a [Twilio](https://www.twilio.com) account, buy a phone number. Add `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_FROM_NUMBER`. Leave these blank to run email-only — SMS sends are skipped (not errored) when unconfigured or when a lead lacks `sms_consent`.
4. **Compliance**: keep review requests going to every "sold" lead the same way — no filtering by how satisfied someone seemed (Google's policy) — and don't flip `sms_consent` on for a lead unless they actually agreed to receive texts (TCPA).
